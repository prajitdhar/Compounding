...
def process_decades_compound(dec_list,input_dir,ctype='compound'):
    ...
    if os.path.exists(f"{input_dir}/{ctype}s/{temporal}_{dec_list[0]}_{tag_str}.pkl"):
        print('Reading file')
        complete_df=pd.read_pickle(f"{input_dir}/{ctype}s/{temporal}_{dec_list[0]}_{tag_str}.pkl")
        
    elif os.path.exists(f"{input_dir}/{ctype}s/10_{dec_list[0]}_{tag_str}.pkl") and temporal!=10000:
        ...
        print(f'Reducing to {temporal}')
        complete_df['time']=complete_df['time']-complete_df['time']%temporal

        complete_df=complete_df.groupby(['modifier','head','time','context'])['count'].sum().to_frame().reset_index()
        
        print("Saving file")
        complete_df.to_pickle(f"{input_dir}/{ctype}s/{temporal}_{dec_list[0]}_{tag_str}.pkl")
        ...
        for dec in dec_list:
            ...
            cur_df['time']=dec
            cur_df['time']=cur_df['time']-cur_df['time']%temporal
