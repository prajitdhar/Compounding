...
#                        ridge_model <- train(trainX,trainY,method = "glmnet",metric = "Rsquared",
...
#                                             preProcess = preprocess_list)
 ...
 #                       lasso_model <- train(trainX,trainY,method = "glmnet",metric = "Rsquared",
#                                             trControl = trainControl("cv", number = 10,search="grid",seeds=seeds),tuneGrid = expand.grid(alpha = 1, lambda = lambda),
#                                             preProcess = preprocess_list)
                        
                        
#                        ridge_spearman_model <- train(trainX,trainY,method = "glmnet",metric = "Spearman",
...
#                        lasso_spearman_model <- train(trainX,trainY,method = "glmnet",metric = "Spearman",
...
#                        perf_ridge<-data.frame(corpus=c,tag=t,ppmi=p,setting=a,timespan=i,cutoff=j,impute=im,dataset=r,pattern=k,features=f,y=pr,n=nrow(trainX),seed=s,ml_algo="ridge",method=getTrainPerf(ridge_model)[,"method"],TrainRsquared=getTrainPerf(ridge_model)[,"TrainRsquared"],TrainSpearman=getTrainPerf(ridge_spearman_model)[,"TrainSpearman"])
#                        perf_lasso<-data.frame(corpus=c,tag=t,ppmi=p,setting=a,timespan=i,cutoff=j,impute=im,dataset=r,pattern=k,features=f,y=pr,n=nrow(trainX),seed=s,ml_algo="lasso",method=getTrainPerf(lasso_model)[,"method"],TrainRsquared=getTrainPerf(lasso_model)[,"TrainRsquared"],TrainSpearman=getTrainPerf(lasso_spearman_model)[,"TrainSpearman"])
...
#                        varimp_ridge<-data.frame(corpus=c,tag=t,ppmi=p,setting=a,timespan=i,cutoff=j,impute=im,dataset=r,pattern=k,features=f,y=pr,n=nrow(trainX),seed=s,ml_algo="ridge",t(varImp(ridge_model)$importance))
#                        varimp_lasso<-data.frame(corpus=c,tag=t,ppmi=p,setting=a,timespan=i,cutoff=j,impute=im,dataset=r,pattern=k,features=f,y=pr,n=nrow(trainX),seed=s,ml_algo="lasso",t(varImp(lasso_model)$importance))
...
rsquared_df<-bind_rows(list_of_rsqr)
rsquared_df$cutoff<-as.factor(rsquared_df$cutoff)
write.csv(rsquared_df,"~/rsquared_google_PPMI_Agnostic.csv",row.names = FALSE)


varimp_10000_df<-bind_rows(list_of_vi_10000)
varimp_10000_df$cutoff<-as.factor(varimp_10000_df$cutoff)
varimp_10000_df[is.na(varimp_10000_df)] <- 0
write.csv(varimp_10000_df,"~/varimp_10000_google_PPMI_Agnostic.csv",row.names = FALSE)

varimp_10_df<-bind_rows(list_of_vi_10)
varimp_10_df$cutoff<-as.factor(varimp_10_df$cutoff)
varimp_10_df[is.na(varimp_10_df)] <- 0
write.csv(varimp_10_df,"~/varimp_10_google_PPMI_Agnostic.csv",row.names = FALSE)


varimp_20_df<-bind_rows(list_of_vi_20)
varimp_20_df$cutoff<-as.factor(varimp_20_df$cutoff)
varimp_20_df[is.na(varimp_20_df)] <- 0
write.csv(varimp_20_df,"~/varimp_20_google_PPMI_Agnostic.csv",row.names = FALSE)

varimp_50_df<-bind_rows(list_of_vi_50)
varimp_50_df$cutoff<-as.factor(varimp_50_df$cutoff)
varimp_50_df[is.na(varimp_20_df)] <- 0
write.csv(varimp_50_df,"~/varimp_50_google_PPMI_Agnostic.csv",row.names = FALSE)


varimp_100_df<-bind_rows(list_of_vi_100)
varimp_100_df$cutoff<-as.factor(varimp_100_df$cutoff)
varimp_100_df[is.na(varimp_100_df)] <- 0
write.csv(varimp_100_df,"~/varimp_100_google_PPMI_Agnostic.csv",row.names = FALSE)
